...
const NotePreview = ({ note, onClose }) => {
  ...
  useEffect(() => {
    const loadPreviewUrl = async () => {
      try {
        // Use the URL directly from the note object
      ...
      <div className="bg-white rounded-xl shadow-2xl p-4 w-[95%] max-w-7xl h-[80vh] flex flex-col mt-15">
        ...
        <div className="flex justify-between items-center mb-2">
          <div>
            ...
            {note.subject && (
              <p className="text-sm text-blue-600">{note.subject.name}</p>
            )}
          </div>
          <button
            onClick={onClose}
        ...
        <div className="relative flex-1 overflow-auto h-full">
          {loading && (
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="animate-spin rounded-full h-10 w-12 border-b-2 border-blue-600"></div>
            </div>
          )}
          {error && (
